...
var webApp = function() {
    ...
    var uiInit = function () {
        ...
        // 40px (height of header) + 40px (padding top + bottom of #page-content) + 31px (height of footer) = 111px
        var pageContent = $('#page-content');
        
        pageContent.css('min-height', $(window).height() - 111 + 'px');
            ...
            if (link.next('ul').length > 0) {
                if (link.parent().hasClass('active') !== true) {
                    if (link.hasClass('open')) {
                        link.removeClass('open').next().slideUp(300);
                    }
                    else {
                        $('#primary-nav li > a.open').removeClass('open').next().slideUp(300);
                        link.addClass('open').next().slideDown(300);
                    }
                }
    ...
    var scrollToTop = function() {
        ...
        // Effect on hover
        link.mouseover(function(){ $(this).animate({opacity: 1}, 150); }).mouseout(function(){ $(this).animate({ opacity: .3 }, 150); });
